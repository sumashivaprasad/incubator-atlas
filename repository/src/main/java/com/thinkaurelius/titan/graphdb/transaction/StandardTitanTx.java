...
public class StandardTitanTx extends TitanBlueprintsTransaction implements TypeInspector, SchemaInspector, VertexFactory {
    ...
    public synchronized void commit() {
        ...
        try {
            if (hasModifications()) {
                log.debug("Graph commit of modifications started");
                graph.commit(addedRelations.getAll(), deletedRelations.values(), this);
                log.debug("Graph commit of modifications finished");
            } else {
                log.debug("Transaction commit started");
                txHandle.commit();
                log.debug("Transaction commit ended");
            }
            success = true;
        } catch (Exception e) {
            try {
