...
//    }
    ...
    public void commit(final Collection<InternalRelation> addedRelations,
        ...
        log.debug("Assigning ids {}", addedRelations.size());
        ...
        try {
            ...
            if (hasSchemaElements && !hasTxIsolation) {
                 ...
                 */
                log.debug("Has schema elements but no transaction isolation ");
            ...
            log.debug("Before prepare commit ");
            ...
            log.debug("After prepare commit ");
            ...
            if (commitSummary.hasModifications) {
                ...
                try {
                    ...
                    log.debug("Committed to storage");
                } catch (Throwable e) {
                ...
                if (hasSecondaryPersistence) {
                    log.debug("Starting for mixed index");
                    ...
                    try {
                        ...
                        if (logTxIdentifier!=null) {
                            log.debug("Logging txn for indexes");
                            try {
                    ...
                    log.debug("2nd commit indexes started");
                    mutator.commitIndexes();
                    log.debug("2nd commit indexes finished");
                }
                ...
                log.debug(" Just mutator commit started ");
                mutator.commit();
                log.debug(" Just mutator commit finished ");
            }
        } catch (Throwable e) {
