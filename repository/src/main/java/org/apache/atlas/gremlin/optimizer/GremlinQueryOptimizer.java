...
/**
 * Optimizer for gremlin queries.  Moves has expressions out
 * of and and converts ors into unions.  This way, the graph indices will be used
 * when evaluating has predicates inside of and/or expression.   This also generates
 * functions where it makes sense in order to minimize the overall size of the query.
 */
public final class GremlinQueryOptimizer {
    ...
    /**
     * Optimizes the provided groovy expression by moving has-predicates
     * out of and calls where possible and converting top-level or calls
     * into unions.
     *
     * @param source
     * @return
    ...
    public static boolean isExtractable(GroovyExpression expr) {
        ...
        HasForbiddenType hasForbiddenTypePredicate = new HasForbiddenType();
    ...
    public static GroovyExpression copyWithNewLeafNode(AbstractFunctionExpression expr, GroovyExpression newLeaf) {
        ...
        AbstractFunctionExpression copy = (AbstractFunctionExpression)expr.copy();
        GroovyExpression newCaller = null;
        if (expr.getCaller() == null) {
            newCaller = newLeaf;
        } else {
            newCaller = copyWithNewLeafNode((AbstractFunctionExpression)copy.getCaller(), newLeaf);
        }
        copy.setCaller(newCaller);
        return copy;
    }
